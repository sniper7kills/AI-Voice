...
def create_folders(uuid):
  ...
  output_dir_speech = os.path.join(output_dir, uuid)
...
def create_meta_data(mrs_dir, user_id = False):
  print('→ Creating META Data')

  conn = sqlite3.connect(os.path.join(mrs_dir, "db", "mimicstudio.db"))
  c = conn.cursor()
  ...
  for row in c.execute('SELECT audio_id, prompt, lower(prompt) FROM audiomodel WHERE user_id = "' + user_id + '" ORDER BY length(prompt)'):
    source_file = os.path.join(mrs_dir, "audio_files", user_id, row[0] + ".wav")
...
def main():
  ...
  parser.add_argument('--user_id', required=False, default=False)
...
#     sys.exit("Passed directory is no valid Mimic-Recording-Studio main directory!")
  ...
  create_folders(args.user_id)
  create_meta_data(args.mrs_dir, args.user_id)
